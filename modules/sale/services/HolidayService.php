...
class HolidayService
{
    ...
    public function refundHoliday(array $requestData): bool
    {
        $dbTransaction = Yii::$app->db->beginTransaction();
        try {
            if (!empty($requestData['Holiday']) || !empty($requestData['HolidaySupplier'])) {
                ...
                if ($holiday->load($requestData)) {
                    ...
                    // Holiday Supplier data process
                    $data = self::holidaySupplierProcess($holiday, $requestData['HolidaySupplier']);
                    ...
                    $services[] = [
                        ...
                        'dueAmount' => $holiday->quoteAmount,
                        'paidAmount' => 0,
                        'supplierData' => $data['serviceSupplierData']
                    ];
